...
const DoctorList = () => {
  ...
  const fetchDoctorsBySpecialization = (specialization) => {
    axios
      ...
      .catch((error) => {
        console.error(
          `Error fetching doctors with specialization ${specialization}:`,
          error
        );
        setLoading(false);
        setError(
          `Failed to fetch doctors with specialization ${specialization}. Please try again later.`
        );
      });
  };
  ...
  const handleSearchSubmit = () => {
    const apiEndpoint =
      searchOption === "name" ? "name" : `doctor/${searchOption}`;
  ...
  return (
    <>
      <div className="mb-4 p-2 pt-4 d-flex flex-row justify-content-between">
        <h1 className="mb-3 mx-4">Doctors</h1>
        <div className="d-flex flex-row justify-content-between mb-3">
          <div className="d-flex flex-row mx-4 ">
            <input
              type="text"
              className="form-control mx-4 px-4 "
              placeholder={`Search by ${searchOption}`}
              value={searchTerm}
              onChange={handleSearch}
            />
            <button
              className="bg-btn rounded px-2"
